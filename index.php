			...
			<div class="row p-4 inner-bg bg-light shadow-lg">
				...
				<div class="col-xl-6 col-lg-6 col-md-6 col-sm-12 col-12">
						<div class="display-6 text-center">
							...
							<span class="text-primary fs-3 mt-5">*** Login Here <i class="bi bi-unlock"></i> ***</span>
						</div>
					<div class="mt-4">
						<form method="POST">
						<div class="form-floating mb-3">
							<input type="text" class="form-control bg-light" id="floatingInput" name="user_id" placeholder="User Name" required/>
							<label for="floatingInput">User ID</label>
						</div>
						<div class="form-floating">
							<input type="password" class="form-control bg-light" id="floatingPassword" name="pass" placeholder="********" required/>
...
if(isset($_POST["submit"]))
	{
		...
		if(!empty($_POST['user_id']) && !empty($_POST['pass']))
		{
			...
			$pass = md5($_POST['pass']);
			//$pass = $_POST['pass'];
			$query = mysqli_query($conn, "SELECT * FROM authenticate WHERE user_id='".$user_id."' AND pass='".$pass."' AND status= '1'");
			...
			if($numrows!=0)
			{
				...
				if($user_id == $username && $pass == $password && $role =='1')
				
				{
					//if(date("Y-m-d") < $end)
					//{
					header('Location:cpanel.php? = C Pannel?');				
					//}
				//	else{
				...
				//	}
						...
						elseif($user_id == $username && $pass == $password && $role =='2')
						{
							header('Location:admin_level.php? = Admin Dashboard?');
						}
						elseif($user_id == $username && $pass == $password && $role =='3')
						{
							header('Location:doctor.php? = Doctor Dashboard');
						}
            elseif($user_id == $username && $pass == $password && $role =='4')
            {
              header('Location:employee_1.php? = Employee1 Dashboard');
            }
            elseif($user_id == $username && $pass == $password && $role =='5')
            {
              header('Location:employee_2.php? = Employee2 Dashboard');
            }
				else
				{
